...
class CVAE(object):
    ...
    def __init__(self, height, width, channel, leaning_rate=1e-3):
        ...
        self.z = tf.compat.v1.placeholder(tf.float32, [None, None])

        self.y, self.z_mu, self.z_sigma = None, None, None # will be initialized by 'build_model'
        self.build_model(input=self.x, sample=self.z, ksize=3)

        self.marginal_likelihood = tf.reduce_sum(self.x * tf.math.log(self.y) + (1 - self.x) * tf.math.log(1 - self.y))
        self.kl_divergence = 0.5 * tf.reduce_sum(tf.square(self.z_mu) + tf.square(self.z_sigma) - tf.math.log(1e-8 + tf.square(self.z_sigma)) - 1, 1)

        self.marginal_likelihood = tf.reduce_mean(self.marginal_likelihood)
        self.kl_divergence = tf.reduce_mean(self.kl_divergence)

        self.ELBO = self.marginal_likelihood - self.kl_divergence # Evidence LowerBOund

        self.loss = -self.ELBO

        self.optimizer = tf.compat.v1.train.AdamOptimizer(self.leaning_rate).minimize(self.loss)

        tf.compat.v1.summary.scalar('marginal_likelihood', self.marginal_likelihood)
        tf.compat.v1.summary.scalar('kl_divergence', self.kl_divergence)
        tf.compat.v1.summary.scalar('total loss', self.loss)
        self.summaries = tf.compat.v1.summary.merge_all()

    def build_model(self, input, sample, ksize=3):

        output_in = self.encoder(input=input, ksize=ksize)
        output_den = self.dense(input=output_in, ksize=ksize)
        self.y = self.decoder(input=output_den, ksize=ksize)
        self.x_sample = self.decoder(input=sample, ksize=ksize)
    ...
    def dense(self, input, ksize=3):

        with tf.name_scope('fully_connected') as scope:
            print("Dense (Fully-Connected)")
            [n, h, w, c] = input.shape
            self.fulcon_in = tf.compat.v1.reshape(input, shape=[-1, h*w*c], name="fulcon_in")
            ...
            self.z_mu = self.fully_connected(input=self.fulcon1, num_inputs=int(self.fulcon1.shape[1]), \
                num_outputs=1, activation="None", name="z_mu")
            self.z_sigma = self.fully_connected(input=self.fulcon1, num_inputs=int(self.fulcon1.shape[1]), \
                num_outputs=1, activation="None", name="z_sigma")

            self.sample = self.sample_z(mu=self.z_mu, sigma=self.z_sigma)

            self.fulcon2 = self.fully_connected(input=self.sample, num_inputs=int(self.sample.shape[1]), \
        ...
        return self.fulcon_out

    def decoder(self, input, ksize=3):

        with tf.name_scope('decoder') as scope:
            print("Decode-1")
            self.convt1_1 = self.conv2d_transpose(input=input, stride=2, padding='SAME', \
            ...
            self.convt3_3 = tf.clip_by_value(self.convt3_3, 1e-37, 1e+37)
